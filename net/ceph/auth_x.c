...
static int __ceph_x_encrypt_offset(const struct ceph_crypto_key *secret)
{
	return ceph_crypt_data_offset(secret) +
	       sizeof(struct ceph_x_encrypt_header);
}

static int ceph_x_encrypt_offset(const struct ceph_crypto_key *secret)
{
	return sizeof(u32) + __ceph_x_encrypt_offset(secret);
}
				 ...
				 int data_len)
{
	...
	return sizeof(u32) + ceph_crypt_buflen(secret, encrypt_len);
}
				...
				void *buf, void *end)
{
	...
	if (need & CEPH_ENTITY_TYPE_AUTH) {
		...
		if (xi->secret.type == CEPH_CRYPTO_AES) {
			ret = ceph_x_encrypt(&xi->secret, enc_buf,
					     CEPHX_AU_ENC_BUF_LEN,
					     sizeof(*blob));
			if (ret < 0)
				return ret;
		} else {
			ret = ceph_hmac_sha256(&xi->secret, blob,
					       sizeof(*blob), enc_buf);
			if (ret)
				return ret;
